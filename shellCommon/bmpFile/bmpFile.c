...
// for i in range(256):
//     if (i % 4 == 0): s += '\n'
//     s += '{0x%02X, 0x%02X, 0x%02X, 0x00}, ' % (i, i, i)
//
// print(s)
//
//================================================================

static BitmapQuad staticPalette[256] =
{
...
stdbool BmpWriter::writeFunc(const Matrix<const Pixel>& image, const CharArray& filename, bool disableSlowdown, stdPars(Kit))
{
    ...
    REQUIRE(safeMul(requiredPitch, srcSizeY, requiredArea));

    Space requiredSizeInBytes = 0;
    REQUIRE(safeMul(requiredArea, Space(sizeof(Pixel)), requiredSizeInBytes));

    //----------------------------------------------------------------
    //
    // Convert src if pitch doesn't satisfy BMP requirements.
    //
    //----------------------------------------------------------------
    ...
    if_not (srcMemPitch == requiredPitch)
    {
        ...
        if_not (size_t(requiredSizeInBytes) <= memorySize)
        {
            ...
            memoryPtr = newPtr;
            memorySize = requiredSizeInBytes;
        }

        ////

        Matrix<Pixel> buffer{(Pixel*) memoryPtr, requiredPitch, srcSizeX, srcSizeY, MatrixValidityAssertion{}};
        MATRIX_EXPOSE(buffer);

        auto srcPtr = srcMemPtr;
    ...
    MATRIX_EXPOSE_UNSAFE(usedImage);
    REQUIRE(usedImageMemPitch == requiredPitch);

    ////

    Space headerSize = sizeof(BitmapFileHeader) + sizeof(BitmapInfoHeader);

    REQUIRE(sizeof(Pixel) == 1 || sizeof(Pixel) == 4);

    bool palette = (sizeof(Pixel) == 1);

    ////

    if (palette)
        headerSize += sizeof(BitmapPalette);

    ////

    Space totalSize{};
    REQUIRE(safeAdd(headerSize, requiredSizeInBytes, totalSize));
    ...
    REQUIRE(convertExact(srcSizeX, header.biWidth));
    REQUIRE(convertExact(srcSizeY, header.biHeight));
    header.biPlanes = 1;
    ...
    if (palette)
    {
        COMPILE_ASSERT(sizeof(staticPalette) == sizeof(header.palette));
        memcpy(&header.palette, &staticPalette, sizeof(header.palette));
    }
